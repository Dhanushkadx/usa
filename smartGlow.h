		...
		switch (getState_smartGlow())
		{
			case INIT:{
				...
				{	clearLED();
					...
					analogWrite(pin_Green, 4096);
					analogWrite(pin_Blue, 700);
			...
			case AUTO_PILO:{
				if (prev_mode!=AUTO_PILO)
				{
					...
					analogWrite(pin_Blue, 4096);
					analogWrite(pin_Green, 500);
					Serial.println("LED AUTO");					
				}
				flickering(number_of_flickering,0,500,4096);
				}break;
